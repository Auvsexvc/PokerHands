namespace Ranking_Sortable_Poker_Hands
{
    public class PokerHand : IComparable<PokerHand>
    {
        ...
        public void SetTheRank()
        {
            ...
            {
                rank += 100000 + (10 * (handCards.Select(c => c.Value).Sum() - handCards.Select(c => c.Value).Distinct().Sum()));
            }
            else
            {
                rank = 0;
        ...
        private static List<Card> HandBuilder(string hand) =>
            hand.Split(" ").Select(c => new Card(c)).ToList();

        public bool IsPair() =>
            handCards.Select(c => c.Value).Distinct().Count() == handCards.Count - 1
                && handCards.Select(c => c.Value).Distinct().Any(c => handCards.Count(d => d.Value == c) == 2);
        ...
        public bool IsThreeOfAKind() =>
            handCards.Select(c => c.Value).Distinct().Count() == handCards.Count - 2
                && handCards.Select(c => c.Value).Distinct().Any(c => handCards.Count(d => d.Value == c) == 3);

        public bool IsStraightLow() =>
            handCards.Select(c => c.Value).Distinct().Count() == handCards.Count
                && handCards.Count(c => c.Value == 14) == 1 && handCards.Where(c => c.Value != 14).Max(c => c.Value) == 5 && handCards.Min(c => c.Value) == 2;

        public bool IsStraight() =>
            handCards.Select(c => c.Value).Distinct().Count() == handCards.Count
                && handCards.Max(c => c.Value) - handCards.Min(c => c.Value) == 4;
        ...
        public bool IsFourOfAKind() =>
            handCards.Distinct().Count() == handCards.Count - 3
                && handCards.Select(c => c.Value).Distinct().Any(c => handCards.Count(d => d.Value == c) == 4);
